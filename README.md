...
- **Deck scoring** -- D&D-style stat block (the **CASTER** profile: Consistency, Acceleration, Surge, Toughness, Efficiency, Reach) on a 1-10 scale, derived from simulation metrics and decklist structure. Use `--score` in the CLI or call `compute_deck_score()` programmatically
